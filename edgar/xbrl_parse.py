...
def get_defenition_URI(file_xsd:str, defenition_type:str) -> list:
	defenition_roleURI = list()
	...
	for e in roleType:
		if defenition_type in e.get_text():
			defenition_roleURI.append(e.get('roleuri'))

	assert defenition_roleURI != [], "statement roleURI not found"
	return defenition_roleURI
...
def statement_URI(statement_roleURI:list, statement:str) -> str:
	...
	SE = ['consolidatedstatementsofshareholdersequity', 'statementconsolidatedstatementsofstockholdersequitydeficit', 
	   	  'consolidatedstatementofchangesinstockholdersequity', 'statementofshareholdersequityandothercomprehensiveincome']
...
# Given the list of URI, we return the URI for balance sheet, income statement, or cash flow
def disclosure_URI(disclosure_roleURI:list, table:str) -> str:
	PPE = ['propertyplantandequipment','propertyandequipment','investmentproperties']
	skip = ['comprehensive','details']
	terms = {
		'ppe': PPE,
	...
	# first check will skip the skip and avoid terms, and numbers
	for uri in disclosure_roleURI:
		stipped_uri = get_role_path(uri)
		if any(s in stipped_uri.replace("_", "").replace("-", "").lower() for s in terms['skip']): 
			continue
		if not contains_no_numbers(stipped_uri): 
			continue
		if any(b in stipped_uri.replace("_", "").replace("-", "").lower() for b in terms[table]): 
			return uri
	...
	print("Disclosure roleURI not found.")
	assert False, f"{terms[table]} not found."
...
def get_disclosure_fields(ticker:str, disclosure, cfiles):
	disclosure_roleURI = get_defenition_URI(cfiles.xsd, 'Disclosure')	
	fs_URI = disclosure_URI(disclosure_roleURI, disclosure)
	return fs_URI
...
if __name__ == "__main__":
	...
	ppe_fields = {}
	ppe_fields = pre_data(ticker, cfiles.pre, ppe_URI, ppe_fields)
	ppe_fields = cal_data(ticker, cfiles.cal, ppe_URI, ppe_fields)
